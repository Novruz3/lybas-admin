...
function TextSlider() {
  const data = useSelector((state) => state?.TextSlider?.data);
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const lang = localStorage.getItem("lang");

  useEffect(() => {
    if (!data?.length) dispatch(fetchTextSlider());
  }, []);

  const deleteTextSlider = async (id) => {
    try {
      const res = await AxiosCustom("/back/text-slider/" + id, {
        method: "DELETE",
      });
      if (res.status === 200) {
    ...
    <div className="categories">
      {!data && (
        <Search
          title="textSlider"
          className="mt-5"
          action={{ link: "textSlider_add", text: "addTextSlider" }}
        />
      )}
                    ...
                    <TableRow hover role="checkbox" tabIndex={-1}>
                      <TableCell align={"center"}>
                        <div className="name font-semibold">
                          {data?.text_slider?.description_en}
                        </div>
                      </TableCell>
                      <TableCell align={"center"}>
                        <button
                          className="mr-3"
                          onClick={() =>
                            deleteTextSlider(data?.text_slider?.id)
                          }
                        >
                          <svg
                            width="24"
