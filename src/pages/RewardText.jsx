...
function RewardText() {
  const data = useSelector((state) => state?.RewardText?.data);
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const lang = localStorage.getItem("lang");

  useEffect(() => {
    if (!data?.length) dispatch(fetchRewardText());
  }, []);

  const deleteRewardText = async (id) => {
    try {
      const res = await AxiosCustom("/back/reward-text/" + id, {
        method: "DELETE",
      });
      if (res.status === 200) {
    ...
    <div className="categories">
      {!data && (
        <Search
          title="rewardText"
          className="mt-5"
          action={{ link: "rewardText_add", text: "addRewardText" }}
        />
      )}
                    ...
                    <TableRow hover role="checkbox" tabIndex={-1}>
                      <TableCell align={"center"}>
                        <div className="name font-semibold">
                          {data?.reward_text?.description_en}
                        </div>
                      </TableCell>
                      <TableCell align={"center"}>
                        <button
                          className="mr-3"
                          onClick={() =>
                            deleteRewardText(data?.reward_text?.id)
                          }
                        >
                          <svg
                            width="24"
