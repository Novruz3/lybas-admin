...
import { fetchRewards } from "../redux/features/Rewards";
...
function RewardsAdd() {
  ...
  const handleUploadImage = async (event) => {
    ...
    try {
      const response = await AxiosCustom(
        url + "back/images?image_type=diplom",
        { method: "POST", data: formData },
        true
      );
      console.log(response);
      setData({ image: response.data.image });
    } catch (error) {
  ...
  const sendData = async (e) => {
    ...
    console.log(data);
    try {
      const res = await AxiosCustom("/back/diploms", {
        method: "POST",
        data,
      });
      if (res.data.status) {
  ...
  return (
    <div className="dress-add">
      <Breadcrumb page={"rewards"} pageLink={"rewards"} name={t("rewardAdd")} />
      <div>
